...
"""
plt.plot(data[['Temperature', 'AxialAxisRmsVibration', 'RadialAxisKurtosis', 'RadialAxisPeakAcceleration',
      'RadialAxisRmsAcceleration', 'RadialAxisRmsVibration']], linewidth=0.8, label=['Temperature', 'AxialAxisRmsVibration', 'RadialAxisKurtosis', 'RadialAxisPeakAcceleration',
      'RadialAxisRmsAcceleration', 'RadialAxisRmsVibration'])
...
split_index_val = int(len(data) * 0.9)  # 10% for validation, remaining 10% for testing
...
def create_model(inputShape, epochs, batch_size):
    model = Sequential([
        GRU(units=100, return_sequences=True, input_shape=inputShape),
        Dropout(0.2),
...
"""
model_1, L = create_model((sequence_length, 1), 32, 16)
...
plt.title("GRU( ACTUAL VS. PRED. )")
plt.legend()
plt.show()
"""
list = []
for x in range(10):
    model, l = create_model((sequence_length, 1), 2, 16)
    list.append(l)

plt.ylabel("TEST LOSS")
plt.title("GRU LOSS CHANGE IN SAME SETTINGS")
plt.plot(list)
plt.show()
